"""Multi-GPU Z-Image Trainer (Class-based)
...
class ZImageTrainer(BaseTrainer):
    """Trainer for Z-Image model with multi-GPU support (DDP-like)."""
    ...
    def _load_zimage(self):
        """Load Z-Image model and replicate to all GPUs."""
        ...
        with torch.device("meta"):
            base_model = ZImage(z_image_params)
        ...
        # TODO might need to implement ramtorch here and set the model to fp32 (ignore it for now!)
        ...
        print(f"  Z-Image loaded on {self.n_gpus} GPUs ({param_count:,} params each)")
    
    def _load_vae(self):
        """Load VAE model and replicate to all GPUs."""
        print("  Loading VAE...")
    ...
    def _forward_backward_on_gpu(
        ...
        for i in range(0, batch_size, cache_mb):
            with torch.no_grad(), torch.autocast(device_type="cuda", dtype=torch.bfloat16):
                ...
                latent_chunk = vae.encode_for_train(chunk)
        ...
        for mb_idx in range(num_minibatches):
            ...
            with torch.no_grad(), torch.autocast(device_type="cuda", dtype=torch.bfloat16):
                ...
                image_pos_ids = prepare_latent_image_ids(token_lengths, h, w).to(device)
    ...
    def _inference_on_gpu(
        ...
        with torch.no_grad(), torch.autocast(device_type="cuda", dtype=torch.bfloat16):
            ...
            image_pos_ids = prepare_latent_image_ids(offset, h, w).to(device)
            ...
            output_for_decode = vae_unflatten(output, shape)
