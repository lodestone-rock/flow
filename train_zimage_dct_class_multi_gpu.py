"""Multi-GPU Z-Image DCT Trainer (Class-based)

A trainer for Z-Image DCT model using ThreadPoolExecutor for multi-GPU parallelism.
Uses NCCL for gradient all-reduce across GPUs.

Based on train_zimage_class_multi_gpu.py but uses:
- ZImageDCT model with SimpleMLPAdaLN decoder
- 32-channel VAE (autoencoder_c32)
"""
...
# =============================================================================
# Weight Loading Utilities for DCT Model
# =============================================================================

def extend_weights_for_c32(original_state_dict: dict) -> dict:
    """
    Extend x_embedder weights from 16 channels (64 dim) to 32 channels (128 dim).
    Pads with zeros to preserve original model behavior on first 16 channels.
    """
    new_state_dict = {}
    ...
    for key, value in original_state_dict.items():
        if "all_x_embedder.2-1.weight" in key:
            ...
            print(f"  Extended {key}: {value.shape} -> {new_weight.shape}")
        else:
...
def load_zimage_weights_to_dct(dct_model: ZImageDCT, original_state_dict: dict) -> ZImageDCT:
    """
    Load weights from original ZImage model into ZImageDCT model.
    Maps the weight names, extends for 32 channels, and skips new decoder layers (dec_net).
    """
    # First extend weights for 32 channels
    extended_state_dict = extend_weights_for_c32(original_state_dict)
    
    dct_state_dict = dct_model.state_dict()
    loaded_keys = []
    skipped_keys = []
    new_keys = []
    
    # Mapping from original ZImage keys to ZImageDCT keys
    key_mapping = {
    ...
    for orig_key, value in extended_state_dict.items():
        if orig_key in key_mapping:
            new_key = key_mapping[orig_key]
            if new_key in dct_state_dict:
                if dct_state_dict[new_key].shape == value.shape:
                    dct_state_dict[new_key] = value
                    loaded_keys.append(f"{orig_key} -> {new_key}")
                else:
                    skipped_keys.append(f"{orig_key} (shape mismatch)")
            else:
                skipped_keys.append(f"{orig_key} (mapped key not found)")
        elif "all_final_layer" in orig_key:
            skipped_keys.append(f"{orig_key} (replaced by dec_net)")
        elif orig_key in dct_state_dict:
            if dct_state_dict[orig_key].shape == value.shape:
                dct_state_dict[orig_key] = value
                loaded_keys.append(orig_key)
            else:
                skipped_keys.append(f"{orig_key} (shape mismatch)")
        else:
            skipped_keys.append(f"{orig_key} (not found)")
    
    # Find new keys (dec_net)
    for key in dct_state_dict.keys():
        if "dec_net" in key:
            new_keys.append(key)
    
    dct_model.load_state_dict(dct_state_dict)
...
class ZImageDCTTrainer(BaseTrainer):
    """Trainer for Z-Image DCT model with multi-GPU support (DDP-like)."""
    ...
    def _load_zimage(self):
        """Load Z-Image DCT model and replicate to all GPUs."""
        print("  Loading Z-Image DCT...")
        
        # Create DCT params for 32-channel model
        dct_params = ZImageDCTParams()
        #     patch_size=2,
        #     f_patch_size=1,
        #     in_channels=32,  # 32 channels for c32 VAE
        #     dim=3840,
        #     n_layers=30,
        #     n_refiner_layers=2,
        #     n_heads=30,
        ...
        # Create base model
        base_model = ZImageDCT(dct_params)
        
        # Load original ZImage weights and map to DCT model
        ...
        base_model.load_state_dict(original_state_dict, assign=True)
        # base_model = load_zimage_weights_to_dct(base_model, original_state_dict)
        ...
        print(f"  Z-Image DCT loaded on {self.n_gpus} GPUs ({total_params:,} params each)")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Train Z-Image DCT model (multi-GPU)")
    parser.add_argument(
        "--config",
        type=str,
    ...
    trainer = ZImageDCTTrainer(config_path=args.config, device=args.device)
    try:
        trainer.setup()
        trainer.train()
    finally:
