...
def train_chroma(rank, world_size, wrap_models, wrap_config, debug=False):
    ...
    (t5_tokenizer, t5, model) = wrap_models
        ...
        while True:
            ...
            for counter, data in tqdm(
                ...
                if counter == profile_steps and latch and training_config.do_profiling:
                    prof.stop()
                    prof.export_chrome_trace(f"trace_no_barrier_save_{rank}.json")
...
def main(config="training_config_dct.json"):
    ...
    with torch.no_grad():
        ...
        wrap_models = [t5_tokenizer, t5, model]
