...
def train_chroma(rank, world_size, debug=False, json_config="training_config.json"):
    ...
    with torch.no_grad():
        ...
        # ====== LoRA SPECIFIC: Swap linear layers with LoRA variants ======
        lora_module = {
            "full": LinearWithLoRA,
        ...
        # ====== LoRA SPECIFIC: Find LoRA parameters ======
    ...
    # ====== LoRA SPECIFIC: Initialize optimizer once (no layer swapping) ======
    ...
    while True:
        ...
        for counter, data in tqdm(
            ...
            if (counter + 1) % training_config.save_every == 0 and rank == 0:
                model.to("cpu")
                # ====== LoRA SPECIFIC: Save only LoRA weights ======
                full_state_dict = model.state_dict()
                lora_state_dict = {}
                for k, v in full_state_dict.items():
                    if any(keyword in k for keyword in trained_layer_keywords):
